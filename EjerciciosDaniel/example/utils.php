...
class utils{
	...
	{ try
		...
		{ foreach($resenias as $fila)
			...
			$valoracion = $fila[2];
			$comentarios = $fila[3];
			self::muestraResenia($idProducto, $nombre, $fecha, $valoracion, $comentarios);
			}
		}else
		{ self::muestraMenNoResenia();
		}
		}catch(Exception $ex)
		{ die(print_r($ex->getMessage()));
		}
		self::muestraBotonResenia( $idProducto );
		self::getTermBusqueda(!null);
	}
